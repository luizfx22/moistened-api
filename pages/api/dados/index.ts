...
// Validadores
const LeituraValidator = yup.object().shape({
...
export default async (req: NextApiRequest, res: NextApiResponse) => {
	...
	const valid = await LeituraValidator.validate(req.body).catch((e) => res.status(500).json({ ...e }));
	if (!valid) return res.status(500).json({ message: "Invalid request!" });

	const { error } = await supabase.from("Leitura").insert([{ ...req.body }]);
